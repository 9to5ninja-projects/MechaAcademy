...
(() => {
    ...
    class Scene_TrainingBattle extends Scene_Battle {
        ...
        initializeTrainingBattle() {
            ...
            // Start first turn immediately since we can't rely on BattleManager
            setTimeout(() => {
                this.startDeckTurn();
            }, 500);
        ...
        setupDeckBattleFlow() {
            // Override turn processing with existence checks
            if (BattleManager.startTurn) {
                this.originalStartTurn = BattleManager.startTurn;
                BattleManager.startTurn = this.startDeckTurn.bind(this);
            } else {
                console.warn('BattleManager.startTurn not found - using alternative approach');
                this.originalStartTurn = null;
            }
            
            if (BattleManager.processTurn) {
                this.originalProcessTurn = BattleManager.processTurn;
                BattleManager.processTurn = this.processDeckTurn.bind(this);
            } else {
                console.warn('BattleManager.processTurn not found - using alternative approach');
                this.originalProcessTurn = null;
            }
            
            // Initialize deck battle state
            this._deckBattlePhase = 'start';
            this._playerTurn = true;
        }

        processDeckTurn() {
            // Handle turn processing for deck-based combat
            const playerMecha = window.MechaCompositeManager?.getActiveMecha();
            
            if (BattleManager._phase === 'input') {
                // Player is selecting cards - handled by showPlayerHand()
                return;
            }
            ...
            // Fallback to original processing for other phases
        ...
        startDeckTurn() {
            ...
            if (this._deckBattlePhase === 'start' && this._playerTurn) {
                // Player turn start
                if (playerMecha) {
                ...
                this.showPlayerHand();
                this._deckBattlePhase = 'input';
                
            } else if (this._deckBattlePhase === 'start' && !this._playerTurn) {
                // Enemy turn
                this.opponent.regenerateResources();
                this.opponent.drawHand();
                this._deckBattlePhase = 'enemy_turn';
                this.processEnemyTurn();
            }
        }
